...
class MiApp(QMainWindow):
	...
	def Inicio(self,):
		"""i=0
		bd=[0,0,0]#Variable para determina si ya logro realizar el ejercicio o no
		if self.ui.lisWid_WorkOut.count() > 0:
			while bd[i]==0:
				if (self.ui.lisWid_WorkOut.findItems("Levanta Brazo Derecho Hasta Arriba",Qt.MatchExactly) and bd[i]==0):
					if self.Var_Elbow_Right>=150:
						bd[i]=1
				if (self.ui.lisWid_WorkOut.findItems("Levanta Brazo Izquierdo Hasta Arriba",Qt.MatchExactly)and  bd[i]==0):
					if self.Var_Elbow_Left>=150:
						bd[i]=1
				if (self.ui.lisWid_WorkOut.findItems("Levanta Brazos Hacia Enfrente",Qt.MatchExactly)and  bd[i]==0):
					if self.Var_Shoulder_Left>=20 and  self.Var_Shoulder_Right>=20:
						bd[i]=1
				if (self.ui.lisWid_WorkOut.findItems("Mueve la cabeza hacia la Izquierda",Qt.MatchExactly)and bd[i]==0):
					if self.Var_Cabeza>=20 and  self.Var_Cabeza>=20:
						bd[i]=1
				if (self.ui.lisWid_WorkOut.findItems("Mueve la cabeza hacia la Derecha",Qt.MatchExactly)and  bd[i]==0):
					if self.Var_Cabeza<=-20 and self.Var_Cabeza<=-20:
						bd[i]=1
				if (self.ui.lisWid_WorkOut.findItems("T-Pose",Qt.MatchExactly)and  bd[i]==0):
					if self.Var_Elbow_Right>=150 and  self.Var_Elbow_Left>=50:
						bd[i]=1
				if (self.ui.lisWid_WorkOut.findItems("Sonrie",Qt.MatchExactly)and bd[i]==0):
					if self.Var_Expresion==0:
						bd[i]=1
			"""
